...
pageWithSidebar(
  ...
  sidebarPanel(width=3,
    ...
    bsTooltip("sample", title = 'Please enter the name of the sample. <b>This string must be present in both provided VCF, PED and text filenames.</b>', 
              placement = "right", options = list(container = "body")),
    ...
    bsTooltip("barcode", 
              title = 'Please enter the sequencing barcode/sample number or similar. <b>This string must be present in both provided VCF, PED and text filenames.</b>', 
              placement = "right", options = list(container = "body")),
  ...
  mainPanel(
    ...
    tabPanel("User upload", value=2,
             ...
             tipify(fileInput("pedFile", "Choose a PED file to upload:", accept = c('text/plain', 'text/plain'), width = "50%"),
                    title = "Select a PED file to upload. Please ensure that this contains a detailed filename with the same content as the VCF file above (i.e. sample name and barcode/run label should be exactly the same in both files).  <b>Example: S196_Exome_001_Stats.ped</b>", 
                    placement = "right", 
                    options = list(container = "body")),
